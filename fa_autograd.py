...
class FeedbackAlignmentFunctionTanh(Function):
    @staticmethod
    def forward(ctx, input, weight, backprop_weight):
    ...
    def backward(ctx, grad_output):
        input, weight, backprop_weight = ctx.saved_variables
        grad_input = grad_weight = grad_backprop_weight = None
        def act_derivative(x): return torch.ones_like(x) - torch.tanh(x) * torch.tanh(x)
        if ctx.needs_input_grad[0]:
            grad_input = (act_derivative(input.mm(weight.t()))
                          * grad_output).mm(backprop_weight)
        if ctx.needs_input_grad[1]:
...
fa_function_tanh = FeedbackAlignmentFunctionTanh.apply


class FeedbackAlignmentTanh(nn.Module):
    def __init__(self, input_features, output_features):
        super(FeedbackAlignmentTanh, self).__init__()
        self.input_features = input_features
        self.output_features = output_features
    ...
    def forward(self, input):
        return fa_function_tanh(input, self.weight, self.backprop_weight)
